package controller;

public enum Constants {
    ;

    public static final int NUMBER_OF_DRIVERS = 20;
    public static final int NUMBER_OF_TEAMS = 10;
    public static final int NUMBER_OF_ENGINES = 4;
    public static final int QUALIFICATION_AWARDED_PLACES = 8;
    public static final int RACE_AWARDED_PLACES = 12;
    public static final double TEAM_COEFFICIENT = 0.8d;
    public static final double ENGINE_COEFFICIENT = 0.2d;
    public static final double INTEREST_RATE = 0.1d;
    public static final double PRICING_PRICE_COEFFICIENT = 0.66155811d;
    public static final double PRICING_POINTS_COEFFICIENT = 0.284929757d;
    public static final double RISK_FACTOR = -0.1d;
    public static final double OVERALL_POINTS_FACTOR = 0.3d;
    public static final double OVERALL_PRICE_CHANGE_FACTOR = 3.75d;
    public static final double OVERALL_PRICE_OFFSET_FACTOR = 3.75d;
    public static final double OVERALL_RISK_FACTOR = -10.0d;
    public static final double OVERALL_RISK_OFFSET = 10d;
    public static final int OFFSET_STAGES = 7;
}
